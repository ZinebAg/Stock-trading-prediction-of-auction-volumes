...
# sampling the data + preparing it 
n_train<-10000
n_test<-nrow(test)
...
# fitting
start_time <- Sys.time()
fit_fgam_all_0 <- fgam(target ~ 1 + pid+day+NLV+LS+lf(relvol)+ lf(absret),data=data.fit[1:n_train,])
end_time <- Sys.time()
...
index_test_start<-n_train+1
index_test_end<-n_train+n_test
newdata2 = data=data.fit[index_test_start:index_test_end,]
dim(newdata2)


predictions_train<-predict(fit_fgam_all_0, newdata = data.fit[1:n_train,])